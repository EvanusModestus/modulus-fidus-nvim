...
function M.execute_request(save_response)
    ...
    -- Parse and display response with pretty JSON or XML
    ...
    -- First pass: collect headers and body
    for _, line in ipairs(lines) do
        ...
        elseif in_body and (line:match("^%s*{") or line:match("^%s*%[") or line:match("^%s*<%?xml")) then
            -- Start collecting body
            body_content = line
        elseif body_content ~= "" then
    ...
    if body_content ~= "" then
        ...
        -- Try XML formatting with xmllint
        if content_type:match("xml") or body_content:match("^%s*<%?xml") then
            local pretty_xml = vim.fn.system("xmllint --format - 2>/dev/null", body_content)
            if vim.v.shell_error == 0 then
                for _, line in ipairs(vim.split(pretty_xml, "\n")) do
                    table.insert(formatted_lines, line)
                end
            else
                -- Fallback to original XML
                for _, line in ipairs(vim.split(body_content, "\n")) do
                    table.insert(formatted_lines, line)
                end
            end
        -- Try JSON formatting with jq
        elseif content_type:match("json") or body_content:match("^%s*[{%[]") then
            local pretty_json = vim.fn.system("jq '.' 2>/dev/null", body_content)
            if vim.v.shell_error == 0 then
